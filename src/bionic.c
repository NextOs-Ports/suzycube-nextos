...
int sc_log_level = 0;   /* SC_LOGCAT=1 mirrors the game's own log */
...
{
    fprintf(stderr, "[sc] abort message: %s\n", m ? m : "(null)");
}
...
static int my_fclose(void *f)               { return f == (void *)sc_sF ? 0 : fclose(xf(f)); }
...
static void my___stack_chk_fail(void)
{
    ...
    void *base = NULL;
    extern const char *sc_mod_at(const void *addr, void **base_out);
    const char *module = sc_mod_at(caller, &base);
    if (module && base)
...
static const unsigned char sc_ctype_tab[1 + 256] = { 0 };
static const unsigned char *sc_ctype_ptr = sc_ctype_tab + 1;
...
extern void *sc_android_sym(const char *name);
extern void *sc_egl_sym(const char *name);
extern void *sc_jni_sym(const char *name);
...
/* O canario do Bionic e' uma VARIAVEL global, nao uma funcao: o codigo do
 * jogo faz ldr do GOT (endereco da variavel) e depois ldr do valor.  Deixar a
 * relocacao sem resolver zera o GOT e o primeiro prologo protegido morre com
 * SIGSEGV em endereco 0 -- foi exatamente o que matou o NativeLoader.load do
 * libmain.so na primeira abertura deste port. */
uintptr_t sc_stack_chk_guard = 0x00000aff0d0a2500ULL;

/* tgkill nao existe na glibc como funcao. */
...
static nx_import tab[] = {
    ...
    E(perror), E(getc), E(putc), E(ungetwc),
    ...
    /* conversion / locale */
    E(atof), E(atoi), E(atol), E(atoll), E(strtol), E(strtoll), E(strtoul),
    ...
    E(wcsftime), E(wctype), E(iswctype), E(wctrans), E(towctrans),
    ...
    E(fork), E(execl), E(waitpid), E(kill), M(tgkill),
    ...
    E(ntohs), E(ntohl), E(inet_ntoa), E(inet_aton),
...
static void *my_dlsym(void *h, const char *sym)
{
    ...
    if (!a) a = sc_android_sym(sym);
    if (!a) a = sc_egl_sym(sym);
    if (!a) a = sc_jni_sym(sym);
...
static int my_dladdr(const void *addr, void *info)
{
    ...
    extern const char *sc_mod_at(const void *addr, void **base_out);
    void *base = NULL;
    const char *name = sc_mod_at(addr, &base);
...
static int my_dl_iterate_phdr(int (*cb)(void *, size_t, void *), void *data)
{
    ...
    extern int sc_iterate_mods(int (*cb)(void *, size_t, void *), void *data);
    return sc_iterate_mods(cb, data);
}
...
size_t sc_bionic_count(void) { return sizeof tab / sizeof *tab; }

nx_import *sc_bionic_entries(size_t *n)
{
