...
static sc_window the_window = { 1, 1280, 720, 1 /* RGBA_8888 */ };
...
void sc_window_set_size(int w, int h)
{
    the_window.width = w;
    the_window.height = h;
...
void *sc_window_handle(void) { return &the_window; }
...
{
    if (w) __atomic_fetch_add(&((sc_window *)w)->refs, 1, __ATOMIC_RELAXED);
}
static void a_window_release(void *w)
{
    if (w) __atomic_fetch_sub(&((sc_window *)w)->refs, 1, __ATOMIC_RELAXED);
}
static int32_t a_window_getWidth(void *w)  { return w ? ((sc_window *)w)->width : 0; }
static int32_t a_window_getHeight(void *w) { return w ? ((sc_window *)w)->height : 0; }
static int32_t a_window_getFormat(void *w) { return w ? ((sc_window *)w)->format : 1; }
...
typedef struct { int32_t refs; } sc_looper;
static __thread sc_looper *tls_looper;
static sc_looper main_looper = { 1 };
...
static void a_looper_acquire(void *l) { if (l) ((sc_looper *)l)->refs++; }
static void a_looper_release(void *l) { if (l) ((sc_looper *)l)->refs--; }
...
const nx_import *sc_android_table(size_t *n)
{
