...
timeout 90 ssh -o BatchMode=yes "root@$DEV" "
  for p in /proc/[0-9]*; do
    exe=\$(readlink \"\$p/exe\" 2>/dev/null)
    case \"\$exe\" in $DST/suzycube*) kill -TERM \${p##*/} 2>/dev/null ;; esac
  done
  n=0
  while [ \$n -lt 20 ]; do
    vivo=0
    for p in /proc/[0-9]*; do
      exe=\$(readlink \"\$p/exe\" 2>/dev/null)
      case \"\$exe\" in $DST/suzycube*) vivo=1 ;; esac
    done
    [ \$vivo -eq 0 ] && break
    sleep 0.5; n=\$((n+1))
  done
  for p in /proc/[0-9]*; do
    exe=\$(readlink \"\$p/exe\" 2>/dev/null)
    case \"\$exe\" in $DST/suzycube*) kill -KILL \${p##*/} 2>/dev/null ;; esac
  done
...
scp -q -o BatchMode=yes "$PORT_DIR/tools/launch_device.sh" \
        "$PORT_DIR/tools/watch_device.sh" "root@$DEV:/tmp/" || exit 1
timeout 60 ssh -o BatchMode=yes "root@$DEV" \
...
timeout $((WARMUP + 60)) ssh -o BatchMode=yes "root@$DEV" "sleep $WARMUP; \
  ps -o pid=,args= 2>/dev/null | grep -i '$DST/suzycube' | grep -v grep || echo 'PROCESSO SUMIU'"

say "medindo com nx-verify"
"$REPO/tools/nx-verify" "$DEV" suzycube --game "$DST/suzycube" --samples 4
rc=$?

say "recolhendo provas"
mkdir -p "$PORT_DIR/verify"
scp -q "root@$DEV:$DST/verify_shot.*.ppm" "$PORT_DIR/verify/" 2>/dev/null || true
scp -q "root@$DEV:$DST/debug.log" "$PORT_DIR/verify/gameplay.log" 2>/dev/null || true
